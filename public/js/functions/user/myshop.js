		...
		user: {
			myShop: {
				...
				init: function() {
					...
					cms.get('user/get_all_my_shop', function(resp) {
					...
					address = cms.isBlank(address) ? cms.user.address : address;
					if(! cms.isBlank(address)) {
				...
				shopInfoHTML: function(obj) {
					var address = cms.isBlank(obj.address) ? '' : ('<br>Address: '+obj.address),
						...
						avatar = cms.isBlank(obj.avatar) ? cms.helper.url.baseUrl('public/img/avatar.png') : obj.avatar;
					return cms.join('')
									...
									('<div class="row-fluid">')
										('<div class="span12">')
											('<div class="pull-right">')
												...
												('<a href="javascript:;" data-shop-id="'+obj.id+'" class="remove-my-list" data-toggle="tooltip" title="Remove from my favorites"><span class="icon-heart"></span></a>')
											('</div>')
										('</div>')
									('</div>')
								('</div>')
