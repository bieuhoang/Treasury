...
class Assets {
	...
	protected $script_dirs   = array('public/js/');
	protected $style_dirs    = array('public/css/');
	protected $cache_dir    = 'public/cache/';
	...
	private function get_raw_links($type, $assets, $media)
	{
		...
		{
			if(substr($asset['path'], 0, 7) == 'http://' || substr($asset['path'], 0, 7) == 'http://')
			{
				$output .= $this->tag($type, $asset['path']);
			}
			else
			{
	...
	private function get_minified_links($type, $assets, $media)
	{
		...
		foreach ($assets as $a)
		{
			if(substr($a['path'], 0, 7) == 'http://' || substr($a['path'], 0, 7) == 'http://')
			{
				$output .= $this->tag($type, $a['path']);
			}
			else
			{
	   ...
	   // ensure we have some content
		...
		switch($type)
		{
			...
			case 'css':
				...
				$config['relativePath'] = base_url($dir) .'/';
