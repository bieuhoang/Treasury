...
class Template {
	...
	{ 
		$this->_mark('Template_Set_Title_start');
		// get title
		$titles = func_get_args();
		// set
		$this->data['site_name'] = implode(' :: ', $titles);
		$this->_mark('Template_Set_Title_end');
	 ...
	 * @return void
	 */
	public function set_yield($name = '') 
	{ 
		$this->_mark('Template_Yield_start');
		...
		if ($name != '')
		{
			$this->current_view = $this->ci->router->directory . $name;
		}
	...
	{
		$this->_mark('Template_Set_Layout_start');
		
		if (!empty($layout_name))
		{
			$this->layout = $this->ci->config->item('OCU_layout_folder') . $layout_name;
		} 
		
		$this->_mark('Template_Set_Layout_end');
		return $this;
	}
